...
var app = getApp();
  ...
  //Shares表新增一条记录，cause是添加原因，cost是金额。
  function addShares(){
          var Shares = AV.Object.extend('Shares');
          var updateCoins = new Shares();
          updateCoins.set('targetUser',AV.Object.createWithoutData('_User', AV.User.current().id));
          updateCoins.save().then(function (todo) {
          }, function (error) {
          });
  ...
  //调用云函数，给打手发送一条模板消息
  function sendModelMsg(formId,openid,game,gamename,templateId){
    Cloud.run('sendPlayOrderTemplateMessage' ,paramsJson).then((data) => {
...
module.exports.sendModelMsg = sendModelMsg; 

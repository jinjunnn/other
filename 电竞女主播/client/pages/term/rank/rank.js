...
Page({
   ...
   */
  data: {
    multiArray: ['倔强青铜','秩序白银','荣耀黄金','尊贵铂金','永恒钻石Ⅴ段','永恒钻石Ⅳ段','永恒钻石Ⅲ段','永恒钻石Ⅱ段','永恒钻石Ⅰ段','至尊星耀Ⅴ段','至尊星耀Ⅳ段','至尊星耀Ⅲ段','至尊星耀Ⅱ段','至尊星耀Ⅰ段', '最强王者1-10星', '最强王者11-20星','最强王者21-30星'],
  ...
  onLoad: function (options) {
    console.log(app.globalData.confi.price.rank)
    this.setData({
      price: app.globalData.confi.price.rank,
      image: app.globalData.confi.indexImage.expertImage
    })
    //旧逻辑
    // r1 = app.globalData.confi.price.rank[0];
    // r2 = app.globalData.confi.price.rank[1];
    // r3 = app.globalData.confi.price.rank[2];
    // r4 = app.globalData.confi.price.rank[3];
    // r5 = app.globalData.confi.price.rank[4];
    // r6 = app.globalData.confi.price.rank[5];
    // r7 = app.globalData.confi.price.rank[6];
    // r8 = app.globalData.confi.price.rank[7];
    // r9 = app.globalData.confi.price.rank[8];
  ...
  bindBuy: function () {
      ...
      price = (this.data.price[this.data.indexEnd] - this.data.price[this.data.indexBegin]);
      console.log(price)
      if (this.data.indexEnd > this.data.indexBegin) {
      wx.showModal({
      title: '微信支付',
  ...
  bindRankBegin(e){
    this.setData({
      indexBegin: Number(e.detail.value)
    })
  },

  bindRankEnd(e){
    this.setData({
      indexEnd: Number(e.detail.value)
    })
  },
  //  这个是原来按星付的逻辑

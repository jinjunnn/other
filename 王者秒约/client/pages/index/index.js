...
Page({
  ...
  bindOpenOtherProgress(e){
    ...
    console.log(e.currentTarget.dataset.urls + e.currentTarget.dataset.objectid)
    wx.navigateToMiniProgram({
      appId: this.data.appId,
      path: e.currentTarget.dataset.urls + e.currentTarget.dataset.objectid,
      extraData: {
        foo: 'bar'
      },
      success(res) {
  ...
  setConfi(objectId){
    ...
    query.get(objectId).then(function (confi) {
         ...
         that.setData({
          ...
          appId:confi.attributes.targetIndexAppId,
        })
    }, function (error) {
      // 异常处理
    });

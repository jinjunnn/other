...
Page({
  ...
  var wechatId = this.data.master.attributes.targetUser.attributes.weChatID
  var combatImages = this.data.master.attributes.combatImages
  console.log(combatImages)
    if (!title) {
    ...
    else if (!combatImages) {
      common.showTip("请上传当前段位截图", "loading");
    }
    else if (!wechatId) {
      common.showTip("请设置微信账号", "loading");
    }
    else if (!gameName) {
  ...
  submitTapNavToProfile(){
    wx.navigateTo({
        url: './pass?objectId='+this.data.master.id
    })

...
Page({
  ...
  onReady: function () {
      wx.setNavigationBarTitle({ title: '预约'+this.data.options.nickname});
  },
  ...
  donate(productDescription,amount,gamename) {
    ...
    Cloud.run('order' ,paramsJson).then((data) => {
      ...
      data.success = () => {
        ...
        //发一条通知
        common.sendNotice(this.data.options.nickname , '您好，收到您的陪打订单，我的游戏昵称是'+this.data.options.nickname+'，请您在游戏中加我好友邀请我陪打，或者请您在个人中心-设置中设置您的正确的游戏昵称，等我加您后邀请您排位。')
        wx.navigateTo({
  ...
  closeConversation(){
          console.log(User.current().attributes.image)
          var image=[{user:this.data.options.user,name:this.data.options.username,image:this.data.options.image},{user:User.current().id,name:User.current().attributes.username,image:User.current().attributes.image}];
          var bill = AV.Object.createWithoutData('_Conversation', CONVERSATION_ID);
          bill.set('image', image);
          bill.save();

                wx.navigateTo({
                url: '../../message/detail/detail?conversationId='+CONVERSATION_ID+'&image='+this.data.options.image
                })
  },
  //添加一条订单记录
  saveOrder(){

...
Page({
  ...
  exchange(){
          //成功后的业务逻辑
          coins = coins - price;
          this.coinsUpdate(coins)
          expenses = expenses - (-this.data.options.price);
          this.expensesUpdate(expenses)
          this.buyRecord()
          this.recordPay()
  ...
  //创建购买订单
    ...
    Cloud.run('order' ,paramsJson).then((data) => {
      ...
      data.success = () => {
        ...
        this.expensesUpdate(expenses)
        //发送一条中奖的消息
        var message = '恭喜您成功加入大神团，请在个人资料中完善资料，我们将尽快与您联系，遥祝您早上荣耀王者。';
  ...
  //User表修改总支出expenses字段
  expensesUpdate(expenses){          
          var todo = AV.Object.createWithoutData('_User', AV.User.current().id);
          todo.set('expenses', expenses);
          todo.save();
  },

  //Record表增加一条交易记录
  recordPay(){  
          payFor = this.data.options.title+'购买'        
          var Record = AV.Object.extend('Record');
          var updateCoins = new Record();
          updateCoins.set('payFor', payFor);
          updateCoins.set('amount',-this.data.options.price);

...
Page({
  ...
  onLoad: function (options) {
    ...
    objectId = this.data.options.objectId,
    payFor = this.data.options.title+this.data.options.content+'皮肤抽奖' 
  },

  onReady: function () {
    wx.setNavigationBarTitle({ title: title+content});
  ...
  bindLucky: function () {
    ...
    if (cost <= coins) {
      ...
      wx.showModal({
      title: '1元抽奖',
      content: '本产品由时代文化集团开发，经腾讯上架审核，请放心抽奖。',
      success: function(res) {
        if (res.confirm) {
            that.donate()
        } else if (res.cancel) {
  ...
  if (pro==1) {
          this.getTimes()
          //发送一条中奖的消息
          var message = '恭喜抽中皮肤'+content+'请加客服人员微信区加微信号：iboo1003 QQ区加QQ号：784092545，加好友后，进入游戏进行索要。微信区好友可能会延迟1-4个小时显示在游戏好友列表中，请耐心等待。';
  ...
  if (times >= price*0.125) {
          ...
          wx.showModal({
            title: '恭喜中奖！',
            content: '请您在48小时内按照指引截图并与微信客服：100099118 取得联系并领取皮肤。如有任何问题请联系客服。',
            success: function(res) {
              if (res.confirm) {
                    wx.navigateTo({
                     url: '../guide/guide'
        ...
        //这里增加一个中奖表。
          ...
          this.getTimes()
          //发送一条中奖的消息
          var message = '恭喜抽中皮肤'+content+'请截屏后加客服人员微信区加微信号：iboo1003 QQ区加QQ号：784092545进行索要。微信区好友可能会延迟1-4个小时显示在游戏好友列表中，请耐心等待。';
          common.sendNotice('皮肤中心' , message)
          wx.showModal({
            title: '恭喜中奖！',
            content: '请您在48小时内按照指引截图并与微信客服：100099118 取得联系并领取皮肤。如有任何问题请联系客服。',
  ...
  donateBuy() {
    ...
    Cloud.run('order' ,paramsJson).then((data) => {
      ...
      data.success = () => {
          //发送一条中奖的消息
          common.sendNotice('皮肤中心' , '恭喜购买皮肤'+content+'请加客服人员微信区加微信号：iboo1003 QQ区加QQ号：784092545，加好友后，进入游戏进行索要。微信区好友可能会延迟1-4个小时显示在游戏好友列表中，请耐心等待。')

...
Page({
  ...
  luckyDraw() {
    coins = coins - cost;
    //概率
    var pro = Math.floor(Math.random() * ((price * t2 * probability).toFixed(0)) + 1);
    console.log(pro)
  ...
  wePayDraw() {
    ...
    var pro = Math.floor(Math.random() * ((price * t1 * probability).toFixed(0)) + 1);
    ...
    if (times >= price * probability * 1000) {
      console.log(times);
  ...
  //增加抽奖数量字段
  totalTimes() {
    var todo = AV.Object.createWithoutData('Property', objectId);

...
Page({
  ...
  bindBuy: function () {
      ...
      if (this.data.trade.attributes.price) {
      wx.showModal({
      title: '微信支付',
      content: this.data.trade.attributes.title+'价格为'+(this.data.trade.attributes.price)+'元，使用微信支付。',
      ...
      wx.showModal({
          title: '兑换确认',
          content: this.data.trade.attributes.title+'的兑换价格为'+(this.data.trade.attributes.price)+'元，推荐您使用钱包余额兑换。',
          success: function(res) {
            if (res.confirm) {
               that.exchange()
            } else if (res.cancel) {
  ...
  exchange(){
          //成功后的业务逻
          this.buyRecord()
          common.amendCoins(-this.data.trade.attributes.price)
          common.amendExpenses(- (-this.data.trade.attributes.price))
          common.addRecord(this.data.trade.attributes.title+'购买' , -this.data.trade.attributes.price)
  ...
  //创建购买订单
    ...
    wx.showToast({
      ...
      amount: this.data.trade.attributes.price * 100,
    }
    Cloud.run('order' ,paramsJson).then((data) => {
      ...
      data.success = () => {
        //成功后的业务逻辑
        this.buyRecord()
        common.addRecord(this.data.trade.attributes.title , -this.data.trade.attributes.price)
        common.amendExpenses(- (-this.data.trade.attributes.price))
        //发送一条中奖的消息
  ...
  buyRecord(){
          ...
          order.set('price',this.data.trade.attributes.price);
